...
R = 50; % Ом
% Параметры UE
ue_Fcarrier = (2500+20/2)*1e6; %QAM 2 %(912.5)*1e6; %QAM 1
ue_Fcarrier = (912.5)*1e6;
P_UE = 1e-12; % Ватт %1e-7 мин UE, 1e-13 мин sens levl
% Параметры 2х помех
f1 = 1812.5e6; %QAM2 %1812.5e6; %QAM %1827.5e6; %Первый
f2 = 2147.5e6; %QAM2 %2690e6; %QAM %2162.5e6; %Первый
...
% A5-4 - 64qam RB-25; A5-7 - 64qam RB-100
index_name = 2;
ue_name = ["A3-4", "A4-5", "A5-4"];
% ue_name = ["A3-7", "A4-8", "A5-7"];
qam_name = ["QPSK", "16QAM", "64QAM"];
ue = lteRMCUL(ue_name(index_name)); % 36.104, A3-7 - NRB = 100, BW = 20 MHz (A4-8, A7-1)
...
% --- Создание помехи IM3
[OFDM_shift_1, OFDM_1_waveform, OFDM_1_freq] = DL_OFDM_on_carrier('3.2', BW_1, 1, f1, resam_fs, P_ofdm(1), R, true);

[OFDM_shift_2, OFDM_2_waveform, OFDM_2_freq] = DL_OFDM_on_carrier('3.2', BW_2, 1, f2, resam_fs, P_ofdm(2), R, true);
...
FOR_SNR_noise_power = bandpower(IM3_signal, resam_fs, [0 resam_fs/2])/R - FOR_SNR_ue_power;
        %SNR
FOR_SNR = FOR_SNR_ue_power/FOR_SNR_noise_power;
fprintf('SNR = %0.2f\n', 10*log10(FOR_SNR*1000));
...
resam_nfft = ue_nfft * ceil(resam_ratio);
nfft_coef = 1;
...
grid
legend('Принятые символы', 'Правильное положение символов');
ylim([-1 1]*1.5);
xlim([-1 1]*1.5);
% set(h, 'Color', 'w');
...
% 
h.Title.String="Сигнальное созвездие "+ qam_name(index_name) + "; SNR = " + round(wtodbm(FOR_SNR),1) + " дБ";
...
subplot(223);
plot(resam_freq/1e6, 10*log10(((abs(fft(OFDM_shift_1,resam_nfft)/resam_nfft).^2)/R*2)*1000) );
xlabel('Частота, МГц'); ylabel('Мощность, дБм');
...
subplot(224);
plot(resam_freq/1e6, 10*log10(((abs(fft(OFDM_shift_2, resam_nfft)/resam_nfft).^2)/R*2)*1000) );
xlabel('Частота, МГц'); ylabel('Мощность, дБм');
...
index_sig = find(ue_Fcarrier-ue_BW/2*1e6 <= resam_freq & resam_freq <= ue_Fcarrier+ue_BW/2*1e6);
plot(resam_freq(index_sig)/1e6, 10*log10(shift_resam_spectrum(index_sig)*1000))
clear index_sig;
% pspectrum(shift_add_resam_waveform,resam_fs);
% xlim(fig_x_lim);
xlim([-1 1]*50 + ue_Fcarrier/1e6);
title(['Спектр поступающий на приемник, SNR = ', num2str(round(wtodbm(FOR_SNR),1)), ' дБ' ]);
% legend('Сигнал ИМ3', 'Сигнал UE');
% xlabel('Частота, МГц'); ylabel('Мощность, дБм');
grid on; hold off;
xlabel('Частота, МГц'); ylabel('Мощность, дБм');
set(findobj(gcf,'type','axes'),'FontSize',12);
